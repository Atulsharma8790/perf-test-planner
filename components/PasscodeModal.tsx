...
export default function PasscodeModal({ onClose, onUnlocked }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const r = await unlock(code.trim()); setLoading(false)
      ...
      <div className="w-full max-w-sm rounded-2xl p-6 space-y-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div><h2 className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>Enter Passcode</h2><p className="text-xs" style={{ color: 'var(--text-dimmer)' }}>Required to generate test plans</p></div>
          </div>
